...
local RAINBOW_COLOR_COUNT = 9

function M.setup_highlights()
  local cfg = require("mmcif-rainbow").config
  vim.api.nvim_set_hl(0, "MmcifRainbow1", { fg = cfg.colors.category })
  for i, color in ipairs(cfg.colors.rainbow) do
    vim.api.nvim_set_hl(0, "MmcifRainbow" .. (i + 1), { fg = color })
  end
  vim.api.nvim_set_hl(0, "MmcifCursorColumn", { bg = "#333333" })
end
...
function M.update(buf)
  ...
  for _, block in ipairs(blocks) do
    ...
    for field_index, field in ipairs(block.field_names) do
      ...
      local token_type = 1 + ((field_index - 1) % RAINBOW_COLOR_COUNT)
    ...
    for _, data_row in ipairs(block.data_rows) do
      for _, vr in ipairs(data_row.value_ranges) do
        if vr.length > 0 then
          local token_type = 1 + (vr.column_index % RAINBOW_COLOR_COUNT)
